...
class NodeClassifier(torch.nn.Module):
    def __init__(self, in_channel, dropout=0.1):
        super().__init__()
        self.fc_1 = torch.nn.Linear(in_channel, 80)
        self.fc_2 = torch.nn.Linear(80, 10)
        self.fc_3 = torch.nn.Linear(10, 1)
    ...
    def forward(self, z):
        ...
        z = self.dropout(z)
        return self.fc_3(z).squeeze(dim=1)
...
decoder = NodeClassifier(in_channel=embedding_dim).to(device)

optimizer = torch.optim.Adam(
    set(memory.parameters()) | set(gnn.parameters())
    | set(decoder.parameters()), lr=0.0001)
...
def train():
    ...
    for batch_id, batch in enumerate(tqdm(train_data.seq_batches(batch_size=200), total=552)):
        ...
        y_pred = decoder(z[assoc[src]]).sigmoid()
...
def test(inference_data):
    ...
    y_pred_all = np.zeros(len(inference_data.y))
    for batch_id, batch in enumerate(tqdm(inference_data.seq_batches(batch_size=200), total=119)):
        ...
        y_pred_all[(batch_id*200):(batch_id*200+len(src))] = y_pred.cpu().numpy()

        memory.update_state(src, pos_dst, t, msg)
        neighbor_loader.insert(src, pos_dst)

    y_true = inference_data.y.cpu().numpy()
