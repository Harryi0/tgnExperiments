...
class DyRepDecoder(torch.nn.Module):
    def __init__(self, embedding_dim, num_surv_samples):
        ...
        self.omega = ModuleList([Linear(in_features=2 * self.embed_dim + link_dim, out_features=1),
                                Linear(in_features=2 * self.embed_dim + link_dim, out_features=1)])
    ...
    def forward(self, all_embeddings, assoc, src, pos_dst, neg_dst_surv,
                neg_src_surv=None, neg_dst=None, last_update=None, cur_time=None, et=None, et_embed=None):

        z_src, z_dst = all_embeddings[assoc[src]], all_embeddings[assoc[pos_dst]]
        z_neg_dst_surv = all_embeddings[assoc[neg_dst_surv]]

        # link_embedding = et_embed[et]
        link_embedding = self.link_embedding(et)
        et_embed = self.link_embedding(torch.arange(2))
        ...
        if last_update is None:
            ...
            lambda_uv = self.hawkes_intensity(z_src, z_dst, et, link_embedding, td_pos)

            surv_u_0 = self.hawkes_intensity(
                z_src.unsqueeze(1).repeat(1,  self.num_surv_samples, 1).view(-1, self.embed_dim),
                z_neg_dst_surv,
                torch.zeros(len(z_neg_dst_surv)),
                et_embed[0].expand(len(z_neg_dst_surv),  -1), td_neg_dst)
            surv_u_1 = self.hawkes_intensity(
                z_src.unsqueeze(1).repeat(1,  self.num_surv_samples, 1).view(-1, self.embed_dim),
                z_neg_dst_surv,
                torch.ones(len(z_neg_dst_surv)),
                et_embed[1].expand(len(z_neg_dst_surv),  -1), td_neg_dst)
            ...
            if neg_src_surv is not None:
                ...
                surv_v_0 = self.hawkes_intensity(
                    z_neg_src_surv,
                    z_dst.unsqueeze(1).repeat(1,  self.num_surv_samples, 1).view(-1, self.embed_dim),
                    torch.zeros(len(z_neg_src_surv)),
                    et_embed[0].expand(len(z_neg_src_surv),  -1), td_neg_src)
                surv_v_1 = self.hawkes_intensity(
                    z_neg_src_surv,
                    z_dst.unsqueeze(1).repeat(1,  self.num_surv_samples, 1).view(-1, self.embed_dim),
                    torch.ones(len(z_neg_src_surv)),
                    et_embed[1].expand(len(z_neg_src_surv),  -1), td_neg_src)
        ...
        if not self.training:
            ...
            if last_update is None:
                ...
                lambda_uv_neg = self.hawkes_intensity(z_src, z_neg_dst, et, link_embedding, td_neg)
    ...
    def hawkes_intensity(self, z_u, z_v, et_uv, et_embed, td, symmetric=False):
        z_u = z_u.view(-1, self.embed_dim)
        z_v = z_v.view(-1, self.embed_dim)
        et = (et_uv > 0).long()
        et_embed = et_embed.view(-1, link_dim)
...
# optimizer = torch.optim.Adam(
#     set(memory.parameters()) | set(gnn.parameters())
#     | set(dyrep.parameters()) | set(link_embedding.parameters()), lr=0.0001)
...
def train():
    ...
    for batch_id, batch in enumerate(tqdm(train_data.seq_batches(batch_size=200), total=220)):
        optimizer.zero_grad()
        ...
        z = gnn(z, last_update, edge_index, data.t[e_id], dyrep.link_embedding(data.y[e_id]))
        ...
        loss_lambda, loss_surv_u, loss_surv_v, _ = dyrep(z, assoc, src, pos_dst, neg_dst_surv,
                                                         ...
                                                         et=link_type, et_embed=None) #link_embedding(torch.arange(2)).detach()
        ...
        # dyrep.link_embedding(torch.tensor([1], dtype=torch.long)).detach().expand(len(src), -1)
        memory.update_state(src, pos_dst, t, dyrep.link_embedding(link_type).detach())
...
def test(inference_data, return_time_hr=None):
    ...
    for batch_id, batch in enumerate(tqdm(inference_data.seq_batches(batch_size=200), total=53)):
        ...
        z, last_update = memory(n_id)
        z = gnn(z, last_update, edge_index, data.t[e_id], dyrep.link_embedding(data.y[e_id])) # link_embedding(data.y[e_id]).detach()
        ...
        loss_lambda, loss_surv_u, loss_surv_v, cond = dyrep(z, assoc, src, pos_dst, neg_dst_surv,
                                                            ...
                                                            et_embed=None)# link_embedding(torch.arange(2)).detach()
        ...
        for src_c, pos_dst_c, t_c, et_c in zip(src, pos_dst, t, link_type):
            ...
            lambda_t_allsamples = dyrep.hawkes_intensity(embeddings_u, embeddings_v,
                                                         et_c.repeat(num_time_samples),
                                                         embed[et_c].view(1,-1).expand(num_time_samples, -1), all_td_c)
            ...
            # expectation = (torch.from_numpy(np.cumsum(sampled_time_scale)).to(device)/train_td_hr_max) * f_samples
...
for epoch in range(1, epochs+1): #51
    ...
    print(f' Val AP: {test_ap:.4f},  Val AUC: {test_auc:.4f}, Val LOSS: {test_loss:.4f}, Val MAE: {test_mae:.4f}')
...
plt.legend()
plt.title("TGN + Hawkes, social, train")
plt.subplot(1,2,2)
