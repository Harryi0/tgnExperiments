...
class TGNMemory(torch.nn.Module):
    ...
    def __compute_msg__(self, n_id, msg_store, msg_module):
        ...
        if self.contain_raw_msg:
            ...
            t = torch.cat(t, dim=0)
            t_rel = t - self.last_update[src]
            t_enc = self.time_enc(t_rel.to(torch.float32))

            msg = msg_module(self.memory[src], self.memory[dst], None, t_enc)
...
class IdentityMessage(torch.nn.Module):
    ...
    def forward(self, z_src, z_dst, raw_msg, t_enc):
        if raw_msg == None:
            return torch.cat([z_src, z_dst, t_enc], dim=-1)
        else:
