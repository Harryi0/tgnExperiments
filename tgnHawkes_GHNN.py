...
class DyRepDecoder(torch.nn.Module):
    ...
    def hawkes_intensity(self, z_u, z_v, td, symmetric=False):
        ...
        # g += self.alpha*torch.exp(-self.w_t*td_norm)
...
_, _, train_return_hr, train_return_hr_true = get_return_time(train_data)

_, _, val_return_hr, val_return_hr_true  = get_return_time(val_data)

_, _, test_return_hr, test_return_hr_true = get_return_time(test_data)
...
fig.savefig('tgnHawkes_GHNN_train.png')
...
plt.title("val mae")
fig2.savefig('tgnHawkes_GHNN_val.png')
